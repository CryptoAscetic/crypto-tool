...
def request_ok():
    ...
    url = (f"https://www.okx.com/priapi/v1/invest/activity/smart-money/token/page?pageNo=1&pageSize=10&duration=3"
           f"&order=tokenTradingTime&chainId=195&txnSource=1%2C2&t=1727484671680")
            ...
            if (timestamp - int(tokenTradingTime) / 1000) <= diff:
                if transactionAction == "BUY":
                    ...
                    if float(tokenFDV) > tokenFDVMax:
                        ...
                        arr.append("【Tron-买买买】合约创建时间：" + otherStyleTime + "\n\r")
                        ...
                        arr.append("看线地址：<" + "https://dexscreener.com/tron/" + tokenAddress + ">\n\r")
                        note_str = "".join(arr)
                        print(r)
                        print(note_str)
                        # if is_buy:
